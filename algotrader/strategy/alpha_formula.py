...
class AlphaFormula2(Strategy):
    def __init__(self, stg_id=None, stg_configs=None):
        super(AlphaFormula2, self).__init__(stg_id=stg_id, stg_configs=stg_configs)
        self.day_count = 0
        self.order = None
    ...
    def _start(self, app_context, **kwargs):
        ...
        self.opens = PipeLine('Opens', self.bars, input_key='Open')
        self.opens.start(app_context)

        self.closes = PipeLine('Closes', self.bars, input_key='Close')
        self.closes.start(app_context)

        self.volumes = PipeLine('Volumes', self.bars, input_key="Volume")
        self.volumes.start(app_context)

        self.close_to_open = Minus(self.closes, self.opens, PipeLine.VALUE)
        self.close_to_open.start(app_context)

        self.close_to_open_ret = Divides(self.close_to_open, self.opens, PipeLine.VALUE)
        self.close_to_open_ret.start(app_context)

        self.close_to_open_ret_rank = Rank(self.close_to_open_ret, PipeLine.VALUE)
        self.close_to_open_ret_rank.start(app_context)

        self.log_volumes = Log(self.volumes, PipeLine.VALUE)
        self.log_volumes.start(app_context)

        self.delta_log_volumes = Delta(self.log_volumes, length=2, input_key=PipeLine.VALUE)
        self.delta_log_volumes.start(app_context)

        self.delta_log_volumes_rank = Rank(self.delta_log_volumes, input_key=PipeLine.VALUE)
        self.delta_log_volumes_rank.start(app_context)

        #
        self.pair_correlation = PairCorrelation(self.delta_log_volumes_rank, self.close_to_open_ret_rank, 6)
        self.pair_correlation.start(app_context)

        super(AlphaFormula2, self)._start(app_context, **kwargs)

    def _stop(self):
        super(AlphaFormula2, self)._stop()
